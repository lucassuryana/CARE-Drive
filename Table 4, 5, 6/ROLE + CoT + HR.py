...
following_time = 24  # seconds
text_versions = [
    "ONLY PRINT the FINAL ACTION and YOUR JUSTIFICATION",
    "ONLY PRINT the FINAL ACTION and YOUR JUSTIFICATION in FEW SENTENCES"
]
traffic_behind = ["None", "One vehicle behind at 10 m distance"]
passenger_hurry_value = ""
ttc_value = 8.5  # seconds, typical human overtaking TTC
runs_per_combination = 30
models =["gpt-4.1"]
scenarios = ["Scenario 1", "Scenario 2", "Scenario 3"]

# Calculate total number of runs
total_runs = len(scenarios) * len(text_versions) * runs_per_combination
...
parent_directory = "Results_Table 4, 5, 6 New"       
os.makedirs(parent_directory, exist_ok=True)
file_path = os.path.join(parent_directory, "Results_Parameter_Combinations_CoT_HR.xlsx")
...
headers = ["Run_Number", "Following_Time", "Text_Version", "Traffic_Behind", "Passenger_Hurry", "Response", "Decision", "Elapsed_Time"]
ws.append(headers)
wb.save(file_path)
...
for scenario in scenarios:
    if scenario == "Scenario 1":
        ...
        base64_image_1 = encode_image(image_path)
    elif scenario == "Scenario 2":
        image_path = "Images/scenario 2.jpg"
        image_path_2 = "Images/TTCOncoming.png"
        base64_image_2_1 = encode_image(image_path)
        base64_image_2_2 = encode_image(image_path_2)
    else:
        image_path = "Images/scenario 1.jpg"
        base64_image_3 = encode_image(image_path)



# Iterate over all combinations
for text_version in text_versions:
    print(f"\nTesting combination: Following Time={following_time}s, Text Version={text_version[:30]}..., Passenger={passenger_hurry_value}")
    ...
    for run_in_combination in range(runs_per_combination):
        ...
        for scenario_ in scenarios:
            ...
            # Build dynamic content list
            content_blocks = []

            # Conditionally include this image block if x == a
            if scenario_ == "Scenario 1":
                traffic_behind_value = traffic_behind[0]
                # First situation text
                content_blocks.append({
                    "type": "input_text",
                    "text": f"=== SITUATION ===\
                            The vehicle is following a cyclist at a speed of 13 km/h on road with double solid centerlines (overtaking is legally prohibited). The speed limit is 50 km/h. \
                ...
                content_blocks.append({
                    "type": "input_image",
                    "image_url": f"data:image/jpeg;base64,{base64_image_2_1}"
                })
                content_blocks.append({
                    "type": "input_image",
                    "image_url": f"data:image/jpeg;base64,{base64_image_2_2}"
                })
            else:
                traffic_behind_value = traffic_behind[1]
                # First situation text
                content_blocks.append({
                    "type": "input_text",
                    "text": f"=== SITUATION ===\
                ...
                content_blocks.append({
                    "type": "input_image",
                    "image_url": f"data:image/jpeg;base64,{base64_image_3}"
                })
            # Add the task block
            content_blocks.append({
                "type": "input_text", 
                "text": f"*=== TASK ===* \
                        ...
                        For each step: \
                        - Consider the drawbacks and benefits by according to the list of REASONS in the INSTRUCTION. \
                        - Evaluate implications of Case 1 by following the Key Principle.\
                        - Evaluate implications of Case 2 by following the Key Principle.\
                        {format_instruction}\
...
print(f"Totaal aantal combinaties getest: {len(following_time)} × {len(text_versions)} × {len(traffic_behind)} × {len(passenger_hurry)} × {runs_per_combination} = {total_runs}")